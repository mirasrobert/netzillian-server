...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const userBoardCount = user.boards.length

      // Check user's plan
      if (user.plan === 'free' && userBoardCount >= 3) {
        return res.status(403).json({
          error: {
            message:
              'Free plan users cannot create more than 3 boards. Please upgrade your plan',
          },
        })
      } else if (user.plan === 'pro' && userBoardCount >= 10) {
        return res.status(403).json({
          error: {
            message:
              'Pro plan users cannot create more than 10 boards. Please upgrade your plan',
          },
        })
      }
...
router.put('/addMember/:userId', [auth, member], async (req, res) => {
  try {
    ...
    // Check if user's plan
    if (user.plan === 'free' && board.members.length >= 3) {
      return res.status(403).json({
        error: {
          message:
            'Free plan users cannot add more than 3 members to a board. Please upgrade your plan',
        },
      })
    } else if (user.plan === 'pro' && board.members.length >= 10) {
      return res.status(403).json({
        error: {
          message:
            'Pro plan users cannot add more than 7 members to a board. Please upgrade your plan',
        },
      })
    }
